...
export function useCreditSimulator() {
    // 2. VOLVEMOS A LA FORMA SEGURA ANTIGUA DE CARGAR EL STORE
    const { entities: financialEntities } = useEntitiesStore();
    
    // 3. ESTADOS PARA MANEJAR LA CARGA AL BACKEND
    ...
    const saveCurrentSimulation = async (customName) => {
        ...
        // Guardar localmente primero (para que la UI reaccione rápido)
        ...
        saveError.value = '';
        try {
            await saveCreditRequest({ simulation: sim });
        } catch (error) {
