  ...
  <div class="wizard-page">
    <div class="wizard-container">
      ...
      <main class="wizard-body card">
        ...
        <!-- PUBLICIDAD BANNERS -->
        <div v-if="currentStep === 1 && globalPromotions.length > 0" class="promo-banners mb-4">
          <div v-for="promo in globalPromotions" :key="promo.id" class="promo-banner">
            <div class="promo-content">
              <h4>🔥 ¡{{ promo.bankName }} tiene una promoción para ti!</h4>
              <p>Activa: <strong>{{ promo.name }}</strong> y obtén 
                <span v-if="promo.type === 'rate_discount'">un descuento de {{ promo.value }}% en tu tasa.</span>
                <span v-else-if="promo.type === 'capital_discount'">un bono de {{ currency === 'PEN' ? 'S/' : '$' }}{{ promo.value }} para tu auto.</span>
                <span v-else-if="promo.type === 'grace_months'">{{ promo.value }} meses de gracia gratis.</span>
                <span v-else-if="promo.type === 'free_desgravamen'">el seguro de desgravamen a 0%.</span>
              </p>
            </div>
            <button class="btn btn-primary btn-promo" @click="applyGlobalPromo(promo)">Aprovechar</button>
          </div>
        </div>
        ...
        <div v-show="currentStep === 4" class="step-content">
          ...
          <div v-if="availableProducts.length > 0 && selectedProductId !== 'custom'" class="product-promos-section mb-4">
            <template v-for="product in availableProducts" :key="product.id">
              <div v-if="product.id === selectedProductId && product.promotions && product.promotions.length > 0">
                <div v-for="promo in product.promotions" :key="promo.id">
                  <label v-if="promo.active" class="check-card promo-toggle-card mt-2" :class="{ 'promo-active': selectedPromotionId === promo.id }">
                    <input type="radio" :value="promo.id" v-model="selectedPromotionId" @click="selectedPromotionId === promo.id ? selectedPromotionId = null : selectedPromotionId = promo.id" />
                    <div class="check-content">
                      <span class="font-bold text-promo">🎁 Activar Promoción: {{ promo.name }}</span>
                      <span class="text-sm text-muted">
                        <span v-if="promo.type === 'rate_discount'">Descuento de {{ promo.value }}% en la tasa.</span>
                        <span v-else-if="promo.type === 'capital_discount'">Bono de descuento de {{ promo.value }} en el préstamo.</span>
                        <span v-else-if="promo.type === 'grace_months'">{{ promo.value }} meses de gracia gratis.</span>
                        <span v-else-if="promo.type === 'free_desgravamen'">Seguro de desgravamen gratis.</span>
                      </span>
                    </div>
                  </label>
                </div>
...
const { getAllActivePromotions } = useEntitiesStore();
const globalPromotions = ref(getAllActivePromotions());
...
const applyGlobalPromo = (promo) => {
  selectedEntityId.value = promo.bankId;
  setTimeout(() => {
      selectedProductId.value = promo.productId;
      setTimeout(() => {
...
.promo-toggle-card.promo-active {
  border-color: #10b981;
  background-color: rgba(16, 185, 129, 0.1);
}
